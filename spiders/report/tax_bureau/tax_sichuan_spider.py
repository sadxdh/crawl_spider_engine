"""国家税务总局四川省税务局公告爬虫 → entity_government_announcement
旧项目参照: data_crawl_server sichuan_tax_spider.py
"""
...
class SichuanTaxSpider(BaseSpider):
    name = 'report_tax_sichuan'
    ...
    headers = {
        ...
        'Referer': 'https://sichuan.chinatax.gov.cn/col/col19241/index.html?number=A001001&uid=93406&pageNum=4',
        'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko)'
                      ' Chrome/132.0.0.0 Safari/537.36 Edg/132.0.0.0',
    }

    url = 'https://sichuan.chinatax.gov.cn/module/web/jpage/dataproxy.jsp'
    website_source = '国家税务总局四川省税务局'
    ...
    def start_requests(self):
        for page in range(self.start_page, self.end_page + 1):
            ...
            # response = curl_cffi_request(url=url, data=data, headers=self.headers, method="POST")
    ...
    def extract_list(self, response):
        ...
        if html_text:
            ...
            for row in li_text:
                ...
                # response_curl = curl_cffi_request(url=url, headers=self.headers, method="GET")
                # yield from self.get_detail(response_curl, temp)
    ...
    def extract_detail(self, response):
        ...
        content = xpath_parse(html_text, '//div[@id="zoom"] | //div[@class="vF_detail_content"] | //div[@class="rich_media_wrp"]')
        if content:
