"""青海省税务局公告爬虫 → entity_government_announcement
旧项目参照: data_crawl_server qinghai_tax_spider.py
"""
...
class QinghaiTaxSpider(BaseSpider):
    name = 'report_tax_qinghai'
    ...
    url = 'http://qinghai.chinatax.gov.cn/common/search/cfe1d0ebce284eba87168a302f1323a0'
    website_source = '国家税务总局青海省税务局'
    ...
    def extract_detail(self, response):
        ...
        content = xpath_parse(html_text, '//div[@id="main"] | //div[@id="ggnr"]')
    ...
    def extract_attachment(self, response):
        ...
        rows = html_text.xpath('//div[@id="main"]/p[a]/a')
        for row in rows:
    ...
    def get_detail(self, response, data):
        ...
        if response:
            try:
                ...
                md5_value = hash_md5(str(release_time) + title + self.website_source)
