"""国家税务总局河南省税务局公告爬虫 → entity_government_announcement
旧项目参照: data_crawl_server henan_tax_spider.py
"""
...
class HenanTaxSpider(BaseSpider):
    name = 'report_tax_henan'
    ...
    website_source = '国家税务总局河南省税务局'

    def start_requests(self):
        for page in range(self.start_page, self.end_page + 1):
            if page == 1:
                url = 'https://henan.chinatax.gov.cn/xxgk/tzgg/'
            else:
                url = f"https://henan.chinatax.gov.cn/xxgk/tzgg/index_{page - 1}.html"
    ...
    def extract_list(self, response):
        ...
        rows = xpath_parse(result, '//ul[@class="listCon"]/li', return_list=True)
        for row in rows:
    ...
    def extract_detail(self, response):
        ...
        content = xpath_parse(html_text, '//div[@id="mainText"]')
