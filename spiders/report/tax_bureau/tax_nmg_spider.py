"""国家税务总局内蒙古自治区税务局公告爬虫 → entity_government_announcement
旧项目参照: data_crawl_server nmg_tax_spider.py
"""
import hashlib, scrapy
from spiders.base_spider import BaseSpider
from utils.tools import *


class NmgTaxSpider(BaseSpider):
    name = 'report_tax_nmg'
    # data_table = 'entity_government_announcement'
    dedup_fields = ['md5_value']
    custom_settings = {
        'CONCURRENT_REQUESTS': 2, 'DOWNLOAD_DELAY': 1,
        'DOWNLOADER_MIDDLEWARES': {
            'middlewares.proxy_middleware.RandomProxyMiddleware': 510,
        }
    }
    proxy_type = "long_proxy"
    headers = {
        'Accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,'
                  'image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.7',
        'Accept-Language': 'zh-CN,zh;q=0.9,en;q=0.8,en-GB;q=0.7,en-US;q=0.6',
        'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko)'
                      ' Chrome/126.0.0.0 Safari/537.36 Edg/126.0.0.0',
    }

    base_url = 'https://neimenggu.chinatax.gov.cn/xxgk/tzgg/index{}.html'

    def start_requests(self):
        for page in range(self.start_page, self.end_page + 1):
            url = self.base_url.format('') if page == 1 else self.base_url.format(f'_{page}')
            yield scrapy.Request(
                url=url,
                method='GET',
                headers=self.headers,
                callback=self.parse_list,
                errback=self.errback
            )

    def parse_list(self, response):
        result = response.text.encode(response.encoding).decode('utf-8')
        html_text = etree.HTML(result)
        rows = html_text.xpath('//div[@id="snxmtimg"]/ul/li')
        for row in rows:
            title = xpath_parse(row, './a/@title')
            href = xpath_parse(row, './a/@href')
            detail_url = urljoin(response.url, href)
            release_time = xpath_parse(row, './span/text()')
            temp = {'title': title, 'detail_url': detail_url, 'release_time': release_time}
            yield scrapy.Request(
                url=detail_url,
                method='GET',
                headers=self.headers,
                callback=self.get_detail,
                cb_kwargs={'data': temp}
            )

    def get_detail(self, response, data):
        url = data['detail_url']
        title = data['title']
        release_time = data['release_time']
        if response:
            try:
                content = self.parse_detail(response)
                attachment_list = self.parse_attachment(response)
            except Exception as e:
                error_msg = f'url: {url}, 公告详情解析错误: {e}'
                self.log_error(error_msg)
                # send_dd_msg(self.spider_name, '解析失败', error_msg)
            else:
                website_source = '国家税务总局内蒙古自治区税务局'
                md5_value = hash_md5(str(release_time) + title + website_source)

                items = {}
                items['publish_time'] = release_time
                items['announcement_title'] = title
                items['announcement_url'] = url
                items['content'] = content
                items['source'] = website_source
                items['md5_value'] = md5_value
                items['_table'] = 'entity_government_announcement'
                # insert_data('entity_government_announcement', data=item)
                yield items
                yield from self.save_attach_data(attachment_list, md5_value)

    @staticmethod
    def parse_detail(response):
        result = response.text.encode(response.encoding).decode('utf-8')
        html_text = html.fromstring(result)

        content = html_text.xpath('//div[@id="sdecontent"]')[0]
        content = html.tostring(content, encoding='unicode')
        return content

    @staticmethod
    def parse_attachment(response):
        result = response.text.encode(response.encoding).decode('utf-8')
        html_str = etree.HTML(result)
        rows = xpath_parse(html_str, '//div[@id="sdecontent"]//span[contains(text(),"附件")]/../a', return_list=True)
        temp_list = []
        if rows:
            for row in rows:
                attach_title_list = row.xpath('./text()')
                if attach_title_list:
                    attach_title = attach_title_list[0]
                    attach_href = row.xpath('./@href')[0]
                    attach_url = urljoin(response.url, attach_href)

                    attach_md5_value = hash_md5(attach_title + attach_url)
                    temp = {'attach_title': attach_title, 'attach_url': attach_url, 'attach_md5_value': attach_md5_value}
                    temp_list.append(temp)
        return temp_list

    def save_attach_data(self, attach_list, md5_value):
        for attach in attach_list:
            attach_md5_value = attach['attach_md5_value']
            attach_title = attach['attach_title']
            attach_url = attach['attach_url']

            items = {}
            items['announcement_md5_value'] = md5_value
            items['attachment_title'] = attach_title
            items['attachment_url'] = attach_url
            items['md5_value'] = attach_md5_value
            items['_table'] = 'entity_government_announcement_mapping'
            # insert_data('entity_government_announcement_mapping', data=item)
            yield items


    def errback(self, failure):
        self.log_error(f'请求失败: {failure.request.url} — {failure.value}')
