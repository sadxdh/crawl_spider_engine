"""国家税务总局海南省税务局公告爬虫 → entity_government_announcement
旧项目参照: data_crawl_server hainan_tax_spider.py
"""
...
class HainanTaxSpider(BaseSpider):
    name = 'report_tax_hainan'
    ...
    website_source = '国家税务总局海南省税务局'

    def start_requests(self):
        for page in range(self.start_page, self.end_page + 1):
            url = 'https://hainan.chinatax.gov.cn/ssxc_1_5/index{}.html'
    ...
    def extract_list(self, response):
        ...
        rows = xpath_parse(result, '//div[@class="nrlb1-r-t-x"]/ul/li', return_list=True)
        for row in rows:
            title = xpath_parse(row, './a/text()')
            href = xpath_parse(row, './a/@href')
            url = urljoin(response.url, href)
            release_time = xpath_parse(row, './em/text()')
    ...
    def extract_detail(self, response):
        ...
        content = xpath_parse(html_text, '//div[@class="zx-xxxqy-nr"] | //div[@id="img-content"]')
    ...
    def extract_attachment(self, response):
        ...
        rows = html_text.xpath('//div[contains(@class,"fujian")]/ul/ul/li')
        for row in rows:
