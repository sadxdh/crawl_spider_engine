"""国家税务总局厦门市税务局公告爬虫 → entity_government_announcement
旧项目参照: data_crawl_server xiamen_tax_spider.py
"""
import hashlib, re, scrapy
from spiders.base_spider import BaseSpider
from utils.tools import *

class XiamenTaxSpider(BaseSpider):
    name = 'report_tax_xiamen'
    ...
    website_source = '国家税务总局厦门市税务局'

    def start_requests(self):
        for page in range(self.start_page, self.end_page + 1):
            url = 'http://xiamen.chinatax.gov.cn/xmswcms/bsfw/bsfw-sstz/pages/index{}.html'
            url = url.format('') if page == 1 else url.format(page)
    ...
    def extract_list(self, response):
        ...
        rows = xpath_parse(result, '//div[@class="page-nav-right"]/div/div//ul/li', return_list=True)
        for row in rows:
            ...
            release_time = xpath_parse(row, './a/span/text()')
    ...
    def extract_detail(self, response):
        ...
        content = xpath_parse(html_text, '//div[@id="fontzoom"]')
    ...
    def extract_attachment(self, response):
        ...
        rows = html_text.xpath('//div[@id="fontzoom"]//p/a')
        for row in rows:
