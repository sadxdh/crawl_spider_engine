"""国家税务总局宁波市税务局公告爬虫 → entity_government_announcement
旧项目参照: data_crawl_server ningbo_tax_spider.py
"""
...
class NingboTaxSpider(BaseSpider):
    name = 'report_tax_ningbo'
    ...
    url = 'https://ningbo.chinatax.gov.cn/api-gateway/jpaas-publish-server/front/page/build/unit'
    website_source = '国家税务总局宁波市税务局'
    re_reqs = RsRequest()

    def generate_params(self, page):
        params = {
            'webId': 'GL1AZq0tvpJqDvyYhAye4',
            'pageId': 'j33gzgcWiKAip3GQmY2Yx',
    ...
    def start_requests(self):
        for page in range(self.start_page, self.end_page + 1):
            ...
            api_url = f'{self.url}?{urlencode(params)}'
    ...
    def extract_list(self, response):
        result = response.json()['data']['html']
        html_text = etree.HTML(result)
        li_text = xpath_parse(html_text, '//td[@id="newslist_10401"]//tr', return_list=True)
        for row in li_text:
            title = xpath_parse(row, './td[1]/a/@title')
            href = xpath_parse(row, './td[1]/a/@href')
            release_time = xpath_parse(row, './td[2]/font/text()')
