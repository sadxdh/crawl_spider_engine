"""国家税务总局新疆自治区税务局公告爬虫 → entity_government_announcement
旧项目参照: data_crawl_server xinjiang_tax_spider.py
"""
...
class XinjiangTaxSpider(BaseSpider):
    name = 'report_tax_xinjiang'
    ...
    website_source = '国家税务总局新疆税务局'
    rs_reqs = RsRequest()

    def start_requests(self):
        for page in range(self.start_page, self.end_page + 1):
            url = 'https://xinjiang.chinatax.gov.cn/zwgk/tzgg/index{}.html'
            ...
            # )
            response = self.rs_reqs.rs_request(url=url, headers=self.headers)
    ...
    def extract_list(self, response):
        ...
        rows = xpath_parse(result, '//div[@class="list_mb3"]/ul/li', return_list=True)
        for row in rows:
    ...
    def get_detail(self, response, data):
        ...
        if response:
            try:
                ...
                announce_title = soup.select('.c_detail>h1')[0].text.strip()
    ...
    def extract_detail(self, response):
        ...
        content = xpath_parse(html_text, '//div[@class="contents"] | //div[@id="img-content"]')
        if not content:
            return None
    ...
    def extract_attachment(self, response):
        ...
        rows = html_text.xpath('//div[@id="append"]/ul/li')
        for row in rows:
