"""国家税务总局甘肃省税务局公告爬虫 → entity_government_announcement
旧项目参照: data_crawl_server gansu_tax_spider.py
"""
...
class GansuTaxSpider(BaseSpider):
    name = 'report_tax_gansu'
    ...
    website_source = '国家税务总局甘肃省税务局'

    @staticmethod
    def generate_params(page):
        start = 1 if page == 1 else (page - 1) * 30 + 1
        end = 30 if start == 1 else start + 29
        params = {'startrecord': start, 'endrecord': end, 'perpage': '10'}
        return params
    ...
    def extract_attachment(self, response):
        ...
        rows = html_text.xpath('//div[@id="zoom"]/p[(a)]/a[(text())]')
        for row in rows:
    ...
    def start_requests(self):
        for page in range(self.start_page, self.end_page + 1):
            ...
            url = "http://gansu.chinatax.gov.cn/module/web/jpage/dataproxy.jsp?" + urlencode(params)
    ...
    def extract_list(self, response):
        ...
        if html_text:
            ...
            for row in li_text:
                ...
                release_time = xpath_parse(row, './/b/text()')
    ...
    def get_detail(self, response, data):
        ...
        try:
            ...
            error_msg = f'url: {announce_url}, 国家税务总局甘肃省税务局, 公告详情解析错误: {e}'
