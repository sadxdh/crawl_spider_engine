"""国家税务总局浙江省税务局公告爬虫 → entity_government_announcement
旧项目参照: data_crawl_server zhejiang_tax_spider.py
"""
...
class ZhejiangTaxSpider(BaseSpider):
    name = 'report_tax_zhejiang'
    ...
    url = 'https://zhejiang.chinatax.gov.cn/module/web/jpage/dataproxy.jsp'
    website_source = '国家税务总局浙江省税务局'
    ...
    def extract_detail(self, response):
        ...
        content = xpath_parse(html_text, '//div[@class="info-cont"]')
    ...
    def extract_attachment(self, response):
        ...
        rows = html_text.xpath('//div[@class="info-cont"]//p[contains(text(),"附件")]/a')
        for row in rows:
