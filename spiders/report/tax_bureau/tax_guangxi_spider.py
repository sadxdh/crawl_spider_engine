"""国家税务总局广西省税务局公告爬虫 → entity_government_announcement
旧项目参照: data_crawl_server guangxi_tax_spider.py
"""
...
class GuangxiTaxSpider(BaseSpider):
    name = 'report_tax_guangxi'
    ...
    website_source = '国家税务总局广西省税务局'

    def start_requests(self):
        for page in range(self.start_page, self.end_page + 1):
            url = 'https://guangxi.chinatax.gov.cn/xxgk/tzgg/index{}.html'
    ...
    def extract_list(self, response):
        ...
        rows = xpath_parse(result, '//div[@class="lmy_info"]/ul/li[(a)]', return_list=True)
        for row in rows:
    ...
    def extract_detail(self, response):
        ...
        content = xpath_parse(html_text, '//div[@class="m-c-text"]')
    ...
    def extract_attachment(self, response):
        ...
        rows = html_text.xpath('//ul[@class="downfile"]/li')
        for row in rows:
