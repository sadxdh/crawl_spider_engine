"""国家税务总局深圳市税务局公告爬虫 → entity_government_announcement
旧项目参照: data_crawl_server shenzhen_tax_spider.py
"""
...
class ShenzhenTaxSpider(BaseSpider):
    name = 'report_tax_shenzhen'
    ...
    website_source = '国家税务总局深圳市税务局'

    def start_requests(self):
        for page in range(self.start_page, self.end_page + 1):
            url = 'https://shenzhen.chinatax.gov.cn/sztax/xxgk/tzgg/common_list{}.shtml'
    ...
    def extract_list(self, response):
        ...
        rows = xpath_parse(result, '//div[@class="pageList infoList listContent"]//li/h4', return_list=True)
        for row in rows:
    ...
    def extract_attachment(self, response):
        ...
        rows = html_text.xpath('//div[@class="content"]//p[font]//a')
        for row in rows:
