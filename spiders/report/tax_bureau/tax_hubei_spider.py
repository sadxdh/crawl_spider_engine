"""国家税务总局湖北省税务局公告爬虫 → entity_government_announcement
旧项目参照: data_crawl_server hubei_tax_spider.py
"""
from spiders.base_spider import BaseSpider
from utils.tools import *
from utils.ruishu.rs_crawler import RsRequest

class HubeiTaxSpider(BaseSpider):
    name = 'report_tax_hubei'
    ...
    headers = {
        ...
        'Referer': 'http://hubei.chinatax.gov.cn/hbsw/xxgk/tzgg/index.html',
        'Upgrade-Insecure-Requests': '1',
        'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) '
                      'Chrome/132.0.0.0 Safari/537.36 Edg/132.0.0.0',
    }

    website_source = '国家税务总局湖北省税务局'
    rs_reqs = RsRequest()

    def start_requests(self):
        for page in range(self.start_page, self.end_page + 1):
            url = 'http://hubei.chinatax.gov.cn/hbsw/xxgk/tzgg/index{}.html'
            url = url.format('') if page == 1 else url.format(page - 1)
    ...
    def extract_list(self, response):
        result = response.text.encode(response.encoding).decode('gbk')
        result = etree.HTML(result)
        rows = xpath_parse(result, '//div[@class="newsList"]/ul/li', return_list=True)
        for row in rows:
            title = xpath_parse(row, './a/@title')
            if title:
                href = xpath_parse(row, './a/@href')
                url = urljoin(response.url, href)
                release_time = xpath_parse(row, './a/span[@class="newsTime fl"]/text()')
    ...
    def extract_attachment(self, response):
        ...
        if response.encoding:
            ...
            rows = html_text.xpath('//div[@id="mainText"]/div/p/a')
            for row in rows:
