"""福建省税务局公告爬虫 → entity_government_announcement
旧项目参照: data_crawl_server fujian_tax_spider.py
"""
...
class FujianTaxSpider(BaseSpider):
    name = 'report_tax_fujian'
    ...
    def extract_detail(self, response):
        ...
        content = xpath_parse(html_text, '//div[@class="Custom_UnionStyle"] | //div[@class="TRS_Editor"]')
    ...
    def extract_attachment(self, response):
        ...
        rows = html_text.xpath('//div[@class="fjxz_box"]/ul/li')
        for row in rows:
            attach_title = xpath_parse(row, './a/text()')
            attach_href = xpath_parse(row, './a/@href')
    ...
    def start_requests(self):
        for page in range(self.start_page, self.end_page + 1):
            ...
            url = "http://fujian.chinatax.gov.cn/was5/web/search?" + urlencode(params)
    ...
    def get_detail(self, response, data):
        ...
        try:
            ...
            error_msg = f'url: {announce_url}, 国家税务总局福建省税务局, 公告详情解析错误: {e}'
