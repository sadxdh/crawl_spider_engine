...
class DfcfIpofileSpider(BaseSpider):
    """东方财富网除新三板、待上市公告、沪深京A股公告外的所有公告下载"""
    name = 'dfcf_ipofile_spider'
    data_table = 'ipo_pdf'
    ...
    def start_requests(self):
        for page in range(self.start_page, self.end_page + 1):
            """请求ipo文件列表页"""
            for ann_type, ipo_type in self.ipo_file_type.items():
                params = {
                    'cb': '',
                    'sr': '-1',
                    'page_size': '50',
    ...
    def get_urls(self, response, ipo_type):
        """得到每个ipo的数据"""
        ...
        for announcement in announcements_list:
            ...
            codes = announcement['codes']
            columns = announcement['columns']
            i = 1 if len(codes) > 1 else 0
            short_name = codes[i]['short_name']
            stock_code = codes[i]['stock_code']
