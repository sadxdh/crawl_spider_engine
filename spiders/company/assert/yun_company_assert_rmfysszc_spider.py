...
class YunRmfysszcSpider(BaseSpider):
    name = 'yun_company_assert_rmfysszc'
    headers = {
        ...
        try:
            ...
            if "年" in time_str and "月" in time_str and "日" in time_str:
                ...
                if match:
                    ...
                    # 格式化为标准时间格式
                    if int(hour) == 24:
                        standard_time = f"{year}-{int(month):02d}-{int(day):02d} {int(hour-1):02d}:59:59"
                    else:
    ...
    def start_requests(self):
        ...
        for j in [1, 2, 5, 6, 7, 8, 10, 11]:
            for i in range(start_page, end_page + 1):
                ...
                headers['Referer'] = f'https://www1.rmfysszc.gov.cn/projects.shtml'
    ...
    def parse(self, response, *args, **kwargs):
        if response:
            try:
                ...
                response = Selector(text=html_json)
            except Exception as e:
                logger.warning(f"资产商品列表响应错误:{e},数据：{response.text}")
                return None
    ...
    def parse_lot_details(self, response):
        ...
        try:
            ...
            item = {}
            # 拍卖相关字段处理
            item['md5_value'] = hash_md5(str(itemId) + title)
            item['auction_title'] = title  # 拍卖标题
            item['auction_status'] = status  # 拍卖状态
            # 修复数值字段，确保为空时使用None而不是空字符串
            item['start_price'] = price  # 起拍价
            item['price_increase_range'] = None  # 加价幅度
            item['evaluation_price'] = evaluation_price  # 评估价
            item['bid_cycle'] = None  # 竞价周期
            item['bond'] = reverse_price  # 保证金
            item['delay_period'] = None  # 延迟周期
            item['reserve_price'] = None  # 保留价(原数据中未包含此字段)
            item['subject_matter_location'] = unit  # 标的所在地
            item['bid_start_time'] = self.convert_to_standard_datetime(start_time)  # 竞价开始时间
            item['bid_end_time'] = self.convert_to_standard_datetime(end_time)  # 竞价结束时间
            item['disposal_unit'] = self.extract_company_name(title)  # 处置单位
            # 修复：将图片列表转换为逗号分隔的字符串
            item['subject_matter_img'] = ','.join(img_src_list) if img_src_list else ''  # 标的物图片
            item['subject_matter_introduce'] = bdjs_html  # 标的物介绍
            item['subject_matter_attachment'] = ','.join(attachment) if attachment else ''  # 标的物附件
            item['bid_announcement'] = pmgg_html  # 竞买公告
            item['bid_notice'] = jmxz_html  # 竞买需知
            item['source_url'] = url  # 来源
            item['stage'] = stage
            yield item
        except Exception as e:
            logger.warning(f"错误url：{url} 商品详情响应错误:{e},数据：{response.text}")
