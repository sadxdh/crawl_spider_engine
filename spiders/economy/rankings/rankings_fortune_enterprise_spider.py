...
class RankingsFortuneEnterpirseSpider(BaseSpider):
    name = 'economy_rankings_fortune_enterprise'
    ...
    def parse_itme(self, get_response, url):
        """三个模块新数据与旧数据解析不一致，需要严格区分开，目前只抓取适配数据"""
        etree_html = etree.HTML(get_response.body)
