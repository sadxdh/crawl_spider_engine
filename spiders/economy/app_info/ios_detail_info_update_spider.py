...
class AppInfoIosDeteilInfoUpdateSpider(BaseSpider):
    # ios app 详情信息
    name = 'app_info_ios_deteil_info_update'
    data_table = ''
    ...
    def parse_detail(self, response, data):
        ...
        # yield items
        yield from self.get_log(data, md5_value)

    def generate_log_params(self, app_id):
        params = {
            'id': app_id,
            'country_id': '75',
            'page': '1',
        ...
        aes_param = self.aes_params | {"d": 0, "sort": "dd", "num": 10}
        k = self.ddDataSpider.generate_k(params, path="/v1/version/log", n=aes_param, r='get')
        params = params | {'k': k}
        return params

    def get_log(self, data, md5_value):
        en_app_id = data['en_app_id']
        params = self.generate_log_params(en_app_id)
        cookies = self.get_cookies()
        self.cookie_status = True
        response = common_request(
            url='https://api.diandian.com/pc/app/v1/version/log',
            headers=self.headers,
            params=params,
            cookies=cookies,
            proxies_type=True
        ...
        if response:
            yield from self.parse_log(response, md5_value)

    def parse_log(self, response, md5_value):
        ...
        if data is None:
            ...
            for row in result:
                app_name = row['name']
                release_date = timestamp_to_datetime(row['release_time'])
                version = row['version']
                release_note = row['release_note']  # 更新日志
                app_size = row['size']
                temp = {
                    'name': app_name,
                ...
                update_logs.append(temp)

            item_data = {
                'log': json.dumps(update_logs),
                'md5_value': md5_value,
                '_table': 'app_info'
            }
            # update_data('app_info', data, condition=f'md5_value="{md5_value}"')
            yield item_data
