...
class AppInfoIosListUpdateSpider(BaseSpider):
    name = 'app_info_ios_list_update'
    data_table = ''
    dedup_fields = ['en_app_id']
    custom_settings = {
    ...
    def parse_list(self, response, rank_name, brand_name, genre_name):
        ...
        if result.get('code') == 9 or result.get('msg') == '当前账号同时登录的设备数超限' or result.get('msg') == "尚未登录":
            ...
            if data and isinstance(data, dict):
                ...
                if ranks:
                    ...
                    for rank in ranks:
                        en_app_id = rank['id']
                        total_ranking = rank.get('total_ranking')  # 总榜排行
                        category_id = rank.get('category_id')  # 类别 1:应用, 2:游戏
                        category_ranking = rank.get('category_ranking')  # 应用/游戏榜 排行
                        genre_id = rank.get('genre_id')  # 分类
                        genre_ranking = rank.get('genre_ranking')  # 分类排行
