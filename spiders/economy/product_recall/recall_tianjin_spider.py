"""产品召回-天津市监局，来源：https://scjgj.tj.gov.cn/zwgk/tzgg → product_recall"""
...
class TianjinRecallSpider(BaseSpider):
    name = 'economy_recall_tianjin'
    ...
    headers = {
        ...
        'Referer': 'https://scjg.tj.gov.cn/tjsscjdglwyh_52651/xwdt/cpzh/xfpzh/index_1.html',
    ...
    def start_requests(self):
        for page in range(self.start_page, self.end_page + 1):
            if page == 1:
                url = 'https://scjg.tj.gov.cn/tjsscjdglwyh_52651/xwdt/cpzh/xfpzh/index.html'
            else:
                url = f'https://scjg.tj.gov.cn/tjsscjdglwyh_52651/xwdt/cpzh/xfpzh/index_{page - 1}.html'
    ...
    def parse_list(self, response):
        ...
        rows = result.xpath('//ul[@class="news_list"]/li')
        for row in rows:
    ...
    def parse_detail(self, response, parms):
        if response:
            html_node_code = get_node_html(response,
                                           content_xpath='//div[@class="page_info"]',
                                           rm_node_xpath=['//div[@id="articlePlayer"]'],
                                           replace_href_xpath='//p[@class="insertfileTag"]/a')
        else:
            html_node_code = None
