...
from utils.file_kit import html_to_pdf
...
# 海关知识产权行政处罚案件
class CustomsPunishIntellectualPropertySpider(BaseSpider):
    name = 'customs_punish_intellectual_property'
    ...
    def start_requests(self):
        website_list = self.query_website(intellectual_property, region_name=self.region_name)
        ...
        if '.htm' in url:
            ...
            if response:
                try:
                    ...
                    data_data2 = self.save_data2(datas)
                    yield data_data2
                except Exception as e:
    ...
    @staticmethod
    def parse_detail(response, domain):
        temp_list = []
        rs_response = response.text.encode(response.encoding).decode('utf-8')
        result = etree.HTML(rs_response)

        is_file_exist = xpath_parse(result, '//div[@id="easysiteText"]/p/a/text()')
        if is_file_exist:
            # 解析详情页的文件链接
            p_list = xpath_parse(result, '//div[@id="easysiteText"]/p', return_list=True)
            for p in p_list:
                announcement_title = xpath_parse(p, './a/text()')
                announcement_href = xpath_parse(p, './a/@href')
                announcement_url = urljoin(domain, announcement_href)
                temp = {'announcement_title': announcement_title, 'announcement_url': announcement_url}
                logger.info(f'附件解析结果：{temp}')
                if announcement_title:
                    temp_list.append(temp)
        # else:
        #     #  解析详情页为图片，转pdf
        #     announcement_title = xpath_parse(result, '//div[contains(@class,"title")]/h2/text()')
        #     file_name = f'{announcement_title}.pdf'
        #     html_code = get_node_html(response, content_xpath='//div[@id="easysiteText"]',
        #                               replace_src_xpath='//p/img')
        #     pdf_content = html_to_pdf(html_code)
        #     # oss_url = upload_file(pdf_content, file_type='punish', file_name=file_name)
        #     oss_url = ""
        #     temp = {
    ...
    def save_data(self, datas):
        ...
        announcement_title = datas['announcement_title']
        items = {}
        items['publish_date'] = publish_date
        items['title'] = title
        items['punish_type'] = datas['punish_type']
        items['entity_name'] = datas['entity_name']
        items['case_num'] = datas['case_num']
        items['announcement_title'] = announcement_title
        items['announcement_url'] = datas['announcement_url']
        items['md5_value'] = hash_md5(title + publish_date + announcement_title)
        items['web_name'] = datas['web_name']
        items['oss_url'] = datas.get('oss_url')
        # insert_data('customs_punish', data=item)
        return items


    def save_data2(self, datas):
        for data in datas:
            title = data['title']
