...
from spiders.economy.listing_company.generate_cookie import GenerateCookie


class ManageRiskBirdCookies(ManageCookies):
    spider_name = 'risk_bird'
    ...
    def generate_cookie_pool(self):
        ...
        if not exist_key(self.account_pool_key):
            ...
            self.init_account_pool(account_dict, max_req_num=100)
        ...
        for account in account_pool:
            ...
            if not hash_exists(self.cookies_pool_key, phone):
                ...
                if self.checkout_account(account, holiday_crawl=True):
                    cookie = GenerateCookie().login(phone, password)
                    if cookie:
    ...
    def get_cookie(self):
        ...
        if cookie_list:
            return self.random_cookie(cookie_list, lifetime=60*60*5)
        return None
