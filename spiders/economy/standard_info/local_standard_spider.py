...
class LocalStandardSpider(BaseSpider):
    name = 'entity_local_standard'
    ...
    def parse_list(self, response):
        ...
        for row in rows:
            publish_date = timestamp_to_datetime(row['issueDate'])
            implement_date = timestamp_to_datetime(row['actDate'])
            standard_num = row['code']
            standard_name = row['chName']
            standard_status = row['status']
            record_num = row.get('recordNo')
            record_date = timestamp_to_datetime(row['recordDate'])
            detail_id = row['pk']
    ...
    def parse_detail(response):
        ...
        region = xpath_parse(result, '//span[contains(@class,"label-info")]/text()')
