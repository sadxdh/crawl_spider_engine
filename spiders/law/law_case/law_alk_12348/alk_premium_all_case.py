...
# data_crawl_server spider/law/law_case/law_alk_12348/alk_premium_all_case.py
class AlkPremiumAllCaseSpider(BaseSpider):
    name = 'alk_premium_all_case'
    ...
    def start_requests(self):
        area_list = [[k, v] for k, v in case_area_list.case_area_number.items()]
        print(area_list)
        for area_data in area_list:
            if self.end_page == 1 or self.end_page == "1":
                for page in range(int(self.start_page), int(self.end_page) + 1):
                    yield from self.get_list(page, area_data, '全部案例')
            else:
                yield from self.get_allpage(area_data)

    def get_allpage(self, area_data):
        base_url = "https://alk.12348.gov.cn/LawMultiSearch/Search"
        data = {
            "searchField": "案例全文",
            "keywords": "",
        ...
        yield scrapy.FormRequest(
            ...
            dont_filter=True,
            cb_kwargs={'area_data': area_data}
        )

    def parse_all_page(self, response, area_data):
        if response:
            try:
                soup = BeautifulSoup(response.text, 'lxml')
                page = soup.select('[class="pagenav"]>a')[-2]
                page_number = re.findall(r"searchPageNew\('(.*?)'", str(page))[0]
            except Exception as e:
                error_msg = f'url: {response.url}, 列表解析错误: {e}'
                self.log_info(error_msg)
                # send_dd_msg(self.spider_name, '解析失败', error_msg)
                page_number = "1"

            for page in range(int(self.start_page), int(page_number) + 1):
                yield from self.get_list(page, area_data, '全部案例')



    def get_list(self, page, area_data, bond_type):
        base_url = "https://alk.12348.gov.cn/LawMultiSearch/Search"
        data = {
            "searchField": "案例全文",
            "keywords": "",
        ...
        yield scrapy.FormRequest(
            ...
            dont_filter=True,
            cb_kwargs={'bond_type': bond_type, 'area_data': area_data}
        )

    def parse_list(self, response, bond_type, area_data):
        soup = BeautifulSoup(response.text, "lxml")
        for data in soup.select('[class="sortlist"]>table>tbody>tr'):
            ...
            url_list = {'case_url':case_url, 'case_title': case_title, "case_number":case_number, "case_submission_time":case_submission_time, 'case_type':case_type, 'bond_type': bond_type, 'area': area_data[1]}
    ...
    def parse_content(self, response, list_data):
        ...
        if data:
            ...
            item_mains['case_area'] = list_data['area']
            ...
            if content_data['adjudication_result'] != "":
                ...
                self.log_error(f'{self.name} crawling source:{list_data["case_url"]}  不包含结果，被去除')
