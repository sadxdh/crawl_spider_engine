...
# 分红融资  分红影响
class DividendImpactEastmoneySpider(BaseSpider):
    name = 'dividend_impact_eastmoney'
    data_table = ''
    ...
    def parse_details_url(self, response, f13):
        ...
        if f13 == 1 or f13 == "1":
            prefix = "SH"
        details_url = f"https://emweb.securities.eastmoney.com/pc_hsf10/pages/index.html?type=web&code={prefix}{code}&color=b#/fhrz"
    ...
    def parse_details(self, response, base_data):
        ...
        if json_data['result']:
            for data in json_data['result'].get('data', []):
                ...
                impl_plan_profile = data['IMPL_PLAN_PROFILE'] # 分红方案
                equity_record_date = data['EQUITY_RECORD_DATE'] # 股权登记日
                ex_dividend_date = data['EX_DIVIDEND_DATE'] # 除权除息日
                pay_cash_date = data['PAY_CASH_DATE'] # 派息日
                assign_progress = data['ASSIGN_PROGRESS'] # 方案进度
                assign_object = data['ASSIGN_OBJECT'] # 分配对象
                ...
                main_item['impl_plan_profile'] = impl_plan_profile
                main_item['equity_record_date'] = equity_record_date
                main_item['ex_dividend_date'] = ex_dividend_date
                main_item['pay_cash_date'] = pay_cash_date
                main_item['assign_progress'] = assign_progress
                main_item['assign_object'] = assign_object
                main_item['source'] = source
                main_item['md5_value'] = hash_md5(f"{share_code}:{notice_date}")
                main_item['basic_data'] = basic_data
                main_item['_table'] = 'listing_stock_dividend_detail'
                yield main_item
